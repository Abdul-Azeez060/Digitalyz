...
Response (JSON only):`;
      ...
      console.warn(
        "Gemini header mapping failed, falling back to local logic:",
        error
      );
    }
...
Response (JSON only):`;
      ...
      console.warn(
        "Gemini validation failed, falling back to local logic:",
        error
      );
    }
...
Response (JSON only):`;
      ...
      console.warn(
        "Gemini rule suggestion failed, falling back to local logic:",
        error
      );
    }
    ...
    Object.entries(tasksByClient).forEach(([clientId, tasks]) => {
      ...
      if (tasks.length > 1) {
        ...
        suggestions.push({
          ...
          description: `Tasks for client ${
            client?.name || clientId
          } should be executed together for efficiency`,
          //@ts-ignore
    ...
    const highCapacityWorkers = data.workers.filter(
      (w) => w.capacity > 40 && w.maxLoadPerPhase > 8
    );
    if (highCapacityWorkers.length > 0) {
      suggestions.push({
        ...
        description:
          "Prevent overloading high-capacity workers based on maxLoadPerPhase",
...
Response (JSON only):`;
      ...
      console.warn(
        "Gemini query processing failed, falling back to local logic:",
        error
      );
    }
    ...
    if (
      ...
      availableFields.includes("duration")
    ) {
    ...
    if (
      ...
      if (lowerQuery.includes("high")) {
        filteredData = filteredData.filter(
          (item) => item.priority && item.priority >= 4
        );
      }

      if (lowerQuery.includes("low")) {
        filteredData = filteredData.filter(
          (item) => item.priority && item.priority <= 2
        );
      }
    }
    ...
    const skillMatch = lowerQuery.match(
      /skills?\s+(?:with|includes?)\s+(\w+)/i
    );
    if (
      skillMatch &&
      (availableFields.includes("skills") ||
        availableFields.includes("requiredSkills"))
    ) {
      const skill = skillMatch[1];
      filteredData = filteredData.filter((item) => {
        const itemSkills = item.skills || item.requiredSkills || [];
        return (
          Array.isArray(itemSkills) &&
