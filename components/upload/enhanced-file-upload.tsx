  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        const mapping = await aiService.mapHeaders(
          uploadedHeaders,
          expectedHeaders
        );
    ...
    try {
      ...
      switch (fileType) {
        case "clients":
          ...
          dispatch({
            type: "SET_CLIENTS",
            payload: normalizedData as Client[],
          });
          validationErrors = validationService.validateClients(
            normalizedData,
            state.tasks
          );
          break;
        case "workers":
          ...
          dispatch({
            type: "SET_WORKERS",
            payload: normalizedData as Worker[],
          });
          validationErrors = validationService.validateWorkers(
            normalizedData,
            state.tasks
          );
          break;
        case "tasks":
          ...
          validationErrors = validationService.validateTasks(
            normalizedData,
            state.clients,
            state.workers
          );
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              <span className="text-body">
                {uploadProgress < 40
                  ? `Parsing ${fileTypeState.toUpperCase()} file...`
                  : uploadProgress < 60
                  ? "Enhanced validation & relationship checks..."
                  : uploadProgress < 80
                  ? "AI header mapping & normalization..."
        ...
        {uploadStatus === "complete" && (
          <Alert className="border-emerald-200 bg-emerald-50 rounded-2xl">
            ...
            <AlertDescription className="text-emerald-800 font-medium">
              File uploaded successfully! Enhanced validation completed with
              relationship checks and group analysis.
            </AlertDescription>
          </Alert>
        )}
